...
if($id==0){
  ...
  if($S_sub==0){
    $sql="select * from sl_product,sl_psort where S_del=0 and P_del=0 ".$M_pinfo."  $M_info and P_sort=S_id and S_sub=".$id." order by P_top desc,P_order,P_id desc limit ".(($page-1)*12).",12";
  }else{
    $sql="select * from sl_product,sl_psort where S_del=0 and P_del=0 ".$M_pinfo."  $M_info and P_sort=S_id and S_id=".$id." order by P_top desc,P_order,P_id desc limit ".(($page-1)*12).",12";
  }
}
      ...
      s[[
        ...
        <div class=\"thumbnail\"> <a href=\"?type=productinfo&id=".$row["P_id"]."\" title=\"".$row["P_title"]."\" class=\"zoom\"  target=\"_blank\" > <img src=\"".pic(splitx($row["P_pic"],"|",0))."\" alt=\"".$row["P_title"]."\" />
          <div class=\"zoomOverlay\"></div>
          </a> </div>
        <div class=\"article\">
          <h2><a href=\"?type=productinfo&id=".$row["P_id"]."\"  target=\"_blank\" title=\"".$row["P_title"]."\">".$row["P_title"]."</a></h2>
        </div>
        <div class=\"info\" style=\"font-size: 20px;color: #ff0000;line-height:50px\">￥".p($row["P_price"])."</div>
      </li>";
