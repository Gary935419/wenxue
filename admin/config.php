...
if($action=="clear_cache"){
	@removeDir("../conn/file");  //更新插件
	@mkdir("../conn/file",0777,true);
	@removeDir("../conn/plug");  //更新插件
	@mkdir("../conn/plug",0777,true);
	@removeDir("../conn/cache");  //更新缓存
	@mkdir("../conn/cache",0777,true);
	die("success");
}

if($action=="testmail"){
	$info=sendmail("自行提供邮箱test","如果你能看到这封邮件，说明网站的邮件功能运作正常！",$C_email);

	if($info=="success"){
        die("{\"code\":\"success\",\"msg\":\"已向".$C_email."发送测试邮件，请登录邮箱查收！\"}");
    }else{
...
if($action=="creat"){
	...
	$callback=getbody($from."/api/index.php?action=creatapp&sub=$sub&domain=".$_SERVER["HTTP_HOST"],"data=".base64_encode(json_encode($H_data)));
	die($callback);
}
...
if($action=="sitemap"){
	...
	$sitemap=$sitemap."<url><loc><![CDATA[".gethttp().$D_domain."]]></loc><priority>1.00</priority><lastmod>".date("Y-m-d",time())."</lastmod><changefreq>always</changefreq></url>".PHP_EOL;


$sql="select N_id as id,'newsinfo' as type from sl_news where N_del=0
...
union select P_id as id,'productinfo' as type from sl_product where P_del=0
union select S_id as id,'product' as type from sl_psort where S_del=0
union select T_id as id,'text' as type from sl_text where T_del=0";

$result = mysqli_query($conn,  $sql);
if (mysqli_num_rows($result) > 0) {
while($row = mysqli_fetch_assoc($result)) {
        if($C_html==0){
			$sitemap=$sitemap."<url><loc><![CDATA[".gethttp().$D_domain."?type=".$row["type"]."&id=".$row["id"]."]]></loc><priority>0.5</priority><lastmod>".date("Y-m-d",time())."</lastmod><changefreq>daily</changefreq></url>".PHP_EOL;
		}else{
			$sitemap=$sitemap."<url><loc><![CDATA[".gethttp().$D_domain."/".$row["type"]."-".$row["id"].".html]]></loc><priority>0.5</priority><lastmod>".date("Y-m-d",time())."</lastmod><changefreq>daily</changefreq></url>".PHP_EOL;
		}
    }
} 
	...
	die("{\"msg\":\"success\",\"info\":\"网站地图已生成，路径<b>http://".$D_domain."/sitemap.xml</b>，请到各站长平台提交sitemap\"}");
}
...
if($action=="save"){
	...
	$C_epay_url=trim($_POST["C_epay_url"]);
	$C_epay_id=trim($_POST["C_epay_id"]);
	$C_epay_key=trim($_POST["C_epay_key"]);
	...
	$C_jipay_id=trim($_POST["C_jipay_id"]);
	$C_jipay_key=trim($_POST["C_jipay_key"]);
	$jipaytype=$_POST["C_jipay_type"];

	if($jipaytype!=""){
		for ($i=0 ;$i<count($jipaytype);$i++){
			$C_jipay_typex=$C_jipay_typex.$jipaytype[$i].",";
		}
		$C_jipay_typex= substr($C_jipay_typex,0,strlen($C_jipay_typex)-1);
	}

	$C_hupay_id=trim($_POST["C_hupay_id"]);
	$C_hupay_key=trim($_POST["C_hupay_key"]);
	$hupaytype=$_POST["C_hupay_type"];

	if($hupaytype!=""){
		for ($i=0 ;$i<count($hupaytype);$i++){
			$C_hupay_typex=$C_hupay_typex.$hupaytype[$i].",";
		}
		$C_hupay_typex= substr($C_hupay_typex,0,strlen($C_hupay_typex)-1);
	}

	$payjstype=$_POST["C_payjs_type"];

	if($payjstype!=""){
		for ($i=0 ;$i<count($payjstype);$i++){
			$C_payjs_typex=$C_payjs_typex.$payjstype[$i].",";
		}
		$C_payjs_typex= substr($C_payjs_typex,0,strlen($C_payjs_typex)-1);
	}

	$epaytype=$_POST["C_epay_type"];
	if($epaytype!=""){
		for ($i=0 ;$i<count($epaytype);$i++){
			$C_epay_typex=$C_epay_typex.$epaytype[$i].",";
		}
		$C_epay_typex= substr($C_epay_typex,0,strlen($C_epay_typex)-1);
	}

	$regr=$_POST["C_regr"];
	if($regr!=""){
		for ($i=0 ;$i<count($regr);$i++){
			$C_regrx=$C_regrx.$regr[$i].",";
		}
		$C_regrx= substr($C_regrx,0,strlen($C_regrx)-1);
	}
	
	$C_payjs_id=trim($_POST["C_payjs_id"]);
	$C_payjs_key=trim($_POST["C_payjs_key"]);
	$C_payjs_id2=trim($_POST["C_payjs_id2"]);
	$C_payjs_key2=trim($_POST["C_payjs_key2"]);
	...
	$C_qqon2=intval($_POST["C_qqon2"]);
	$C_wxon2=intval($_POST["C_wxon2"]);
	...
	$C_buylimit=intval($_POST["C_buylimit"]);
	$C_viplimit=intval($_POST["C_viplimit"]);

	$C_fzon=intval($_POST["C_fzon"]);
	$C_fzk=intval($_POST["C_fzk"]);
	$C_zzk=intval($_POST["C_zzk"]);
    ...
    $C_fen1=intval($_POST["C_fen1"]);
    $C_fen2=intval($_POST["C_fen2"]);
    $C_fen3=round($_POST["C_fen3"],2);

    $C_postage=round($_POST["C_postage"],2);
    $C_baoyou=round($_POST["C_baoyou"],2);
	...
	if($C_title==""){
		...
		C_jipay_id='$C_jipay_id',
		C_jipay_type='$C_jipay_typex',
		C_hupay_id='$C_hupay_id',
		C_hupay_type='$C_hupay_typex',
		C_epay_url='$C_epay_url',
		C_epay_id='$C_epay_id',
		C_epay_type='$C_epay_typex',
		C_payjs_type='$C_payjs_typex',
		C_payjs_id='$C_payjs_id',
		C_payjs_id2='$C_payjs_id2',
		...
		C_punsh=$C_punsh,
		C_nunsh=$C_nunsh,
		C_buylimit=$C_buylimit,
		C_viplimit=$C_viplimit,
		C_fee=$C_fee,
		C_rzfee=$C_rzfee,
		...
		if($C_jipay_key!=""){
			mysqli_query($conn,"update sl_config set C_jipay_key='$C_jipay_key'");
		}
		if($C_hupay_key!=""){
			mysqli_query($conn,"update sl_config set C_hupay_key='$C_hupay_key'");
		}
		if($C_epay_key!=""){
			mysqli_query($conn,"update sl_config set C_epay_key='$C_epay_key'");
		}
		if($C_payjs_key!=""){
			mysqli_query($conn,"update sl_config set C_payjs_key='$C_payjs_key'");
		}
		if($C_payjs_key2!=""){
			mysqli_query($conn,"update sl_config set C_payjs_key2='$C_payjs_key2'");
		}
		if($C_wx_appsecret!=""){
...
<!--												</li>-->
<!--												<li class="nav-item">-->
<!--													<a class="nav-link" id="contact-tab2" data-toggle="tab" href="#t3" role="tab" aria-controls="contact" aria-selected="false">收款设置</a>-->
<!--												</li>-->
<!--												<li class="nav-item">-->
...
<!--												</li>-->
<!--												<li class="nav-item">-->
<!--													<a class="nav-link" id="contact-tab2" data-toggle="tab" href="#t6" role="tab" aria-controls="contact" aria-selected="false">接口设置</a>-->
<!--												</li>-->
<!--												-->
<!--												<li class="nav-item">-->
...
<!--												</li>-->
											...
											<div class="tab-content tab-bordered" id="myTab2Content">
												...
												<div class="form-group row">
													<label class="col-md-3 col-form-label" >注册登录开关</label>
													<div class="col-md-9">
														<label><input value="1" type="checkbox" name="C_regon" <?php if($C_regon==1){echo "checked='checked'";}?>> 常规注册登录</label><br>
														<label><input value="1" type="checkbox" name="C_dxon" <?php if($C_dxon==1){echo "checked='checked'";}?>> 手机号注册</label><br>
														<label><input value="1" type="checkbox" name="C_qqon" <?php if($C_qqon==1){echo "checked='checked'";}?>> QQ快捷登录（网站端）</label><br>
														<label><input value="1" type="checkbox" name="C_qqon2" <?php if($C_qqon2==1){echo "checked='checked'";}?>> QQ快捷登录（APP端）</label><br>
														<label><input value="1" type="checkbox" name="C_wxon" <?php if($C_wxon==1){echo "checked='checked'";}?>> 微信快捷登录（网站端）</label><br>
														<label><input value="1" type="checkbox" name="C_wxon2" <?php if($C_wxon2==1){echo "checked='checked'";}?>> 微信快捷登录（APP端）</label><br>
												...
												<div class="form-group row">
													<label class="col-md-3 col-form-label" >注册必填项</label>
													<div class="col-md-9">
														<label><input value="mobile" type="checkbox" name="C_regr[]" <?php if(strpos($C_regr,"mobile")!==false){echo "checked='checked'";}?>> 手机</label><br>
														<label><input value="email" type="checkbox" name="C_regr[]" <?php if(strpos($C_regr,"email")!==false){echo "checked='checked'";}?>> 邮箱</label><br>
														<label><input value="qq" type="checkbox" name="C_regr[]" <?php if(strpos($C_regr,"qq")!==false){echo "checked='checked'";}?>> QQ</label><br>
														
														
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*配置收款接口的微信APP ID及微信APP secret即可实现微信快捷登录</div>
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*如果您不知道如何填写，请点击<a href="<?php echo $url_from?>/h2.html" target="_blank">查看帮助</a></div>
													</div>
												</div>
										...
										<div class="card-body">
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
														...
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*如果您不知道如何填写，请点击<a href="<?php echo $url_from?>/h1.html" target="_blank">查看帮助</a> 提供平台：<a href="https://b.alipay.com/index2.htm" target="_balnk">支付宝官网</a></div>
													</div>
												</div>
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
														...
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*如果您不知道如何填写，请点击<a href="<?php echo $url_from?>/h1.html" target="_blank">查看帮助</a> 提供平台：<a href="https://pay.weixin.qq.com" target="_balnk">微信支付官网</a></div>
													</div>
												</div>
												<hr>
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
														...
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*如果您不知道如何填写，请点击<a href="<?php echo $url_from?>/h1.html" target="_blank">查看帮助</a> 提供平台：<a href="https://qpay.qq.com/" target="_balnk">QQ钱包官网</a></div>
													</div>
												</div>
...
<div class="card card-warning">
										...
										<div class="card-body">
										<div class="form-group row">
													<label class="col-md-3 col-form-label" >支付宝</label>
													<div class="col-md-9">
												...
												<div class="form-group row">
													<label class="col-md-3 col-form-label" >微信支付</label>
													<div class="col-md-9">
												...
												<div class="form-group row">
													<label class="col-md-3 col-form-label" >QQ钱包</label>
													<div class="col-md-9">
														<label><input value="1" type="checkbox" name="C_qpayon" <?php if($C_qpayon==1){echo "checked='checked'";}?>> QQ钱包商户 </label>
...
<div class="card card-warning">
										...
										<div class="form-group row">
													...
													<div class="col-md-9">
														...
														<label><input value="1" type="checkbox" name="C_dmfon" <?php if($C_dmfon==1){echo "checked='checked'";}?>> 当面付 </label>
														<label><input value="1" type="checkbox" name="C_7payon" <?php if($C_7payon==1){echo "checked='checked'";}?>> 7支付收款 </label>
														<label><input value="alipay" type="checkbox" name="C_payjs_type[]" <?php if(strpos($C_payjs_type,"alipay")!==false){echo "checked='checked'";}?>> PAYJS </label>
														<label><input value="alipay" type="checkbox" name="C_codepay_type[]" <?php if(strpos($C_codepay_type,"alipay")!==false){echo "checked='checked'";}?>> 码支付</label>
														<label><input value="alipay" type="checkbox" name="C_jipay_type[]" <?php if(strpos($C_jipay_type,"alipay")!==false){echo "checked='checked'";}?>> 极支付</label>

														<!--<label><input value="alipay" type="checkbox" name="C_hupay_type[]" <?php if(strpos($C_hupay_type,"alipay")!==false){echo "checked='checked'";}?>> 虎皮椒</label>-->

														<?php if($config->epay=="true"){?>
														<label><input value="alipay" type="checkbox" name="C_epay_type[]" <?php if(strpos($C_epay_type,"alipay")!==false){echo "checked='checked'";}?>> 易支付</label>
														<?php }?>
													</div>
												</div>
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
														...
														<label><input value="wxpay" type="checkbox" name="C_payjs_type[]" <?php if(strpos($C_payjs_type,"wxpay")!==false){echo "checked='checked'";}?>> PAYJS </label>
														<label><input value="wxpay" type="checkbox" name="C_codepay_type[]" <?php if(strpos($C_codepay_type,"wxpay")!==false){echo "checked='checked'";}?>> 码支付 </label>
														<label><input value="wxpay" type="checkbox" name="C_jipay_type[]" <?php if(strpos($C_jipay_type,"wxpay")!==false){echo "checked='checked'";}?>> 极支付 </label>
														<label><input value="wxpay" type="checkbox" name="C_hupay_type[]" <?php if(strpos($C_hupay_type,"wxpay")!==false){echo "checked='checked'";}?>> 虎皮椒 </label>
														<?php if($config->epay=="true"){?>
														<label><input value="wxpay" type="checkbox" name="C_epay_type[]" <?php if(strpos($C_epay_type,"wxpay")!==false){echo "checked='checked'";}?>> 易支付 </label>
														<?php }?>
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
														...
														<label><input value="qqpay" type="checkbox" name="C_codepay_type[]" <?php if(strpos($C_codepay_type,"qqpay")!==false){echo "checked='checked'";}?>> 码支付 </label>
														<?php if($config->epay=="true"){?>
														<label><input value="qqpay" type="checkbox" name="C_epay_type[]" <?php if(strpos($C_epay_type,"qqpay")!==false){echo "checked='checked'";}?>> 易支付 </label>
														<?php }?>
													</div>
												</div>	
										...
										<div class="card-body">
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
														<textarea  name="C_dmf_key2" class="form-control" rows="2" <?php if($C_dmf_key2!=""){echo "placeholder=\"已加密保存，留空则不修改\"";}?>></textarea>
														<div style="margin-top: 10px;font-size: 12px;color: #ff0000">注意：这里填写的是<b>支付宝公钥</b>不是<b>应用公钥</b></div>
													</div>
												</div>
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
														<textarea  name="C_dmf_key" class="form-control" rows="2" <?php if($C_dmf_key!=""){echo "placeholder=\"已加密保存，留空则不修改\"";}?>></textarea>
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*如果您不知道如何填写，请点击<a href="<?php echo $url_from?>/h23.html" target="_blank">查看帮助</a> 提供平台：<a href="https://www.alipay.com" target="_balnk">支付宝官网</a></div>
													</div>
												</div>
												...
												<?php if($config->epay=="true"){?>
												<p style="text-align: center;font-weight: bold;font-size: 17px">易支付收款</p>
												<div class="form-group row">
													<label class="col-md-3 col-form-label" >易支付网关</label>
													<div class="col-md-9">
														<input type="text"  name="C_epay_url" class="form-control"  value="<?php echo $C_epay_url?>">
													</div>
												</div>
												<div class="form-group row">
													<label class="col-md-3 col-form-label" >易支付商户ID</label>
													<div class="col-md-9">
														<input type="text"  name="C_epay_id" class="form-control"  value="<?php echo $C_epay_id?>">
													</div>
												</div>

												<div class="form-group row">
													<label class="col-md-3 col-form-label" >易支付商户密钥</label>
													<div class="col-md-9">
														<input type="text"  name="C_epay_key" class="form-control"  value="" <?php if($C_epay_key!=""){echo "placeholder=\"已加密保存，留空则不修改\"";}?>>
													</div>
												</div>
												<div class="form-group row">
													...
													<div class="col-md-9">
														...
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*如果您不知道如何填写，请点击<a href="<?php echo $url_from?>/h17.html" target="_blank">查看帮助</a> 提供平台：易支付官网</div>
													</div>
												</div>
												<hr>
												<?php }?>
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
														...
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*如果您不知道如何填写，请点击<a href="<?php echo $url_from?>/h17.html" target="_blank">查看帮助</a> 提供平台：<a href="https://codepay.kjkl8.com/home.htm" target="_balnk">码支付官网</a></div>
													</div>
												</div>
												...
												<p style="text-align: center;font-weight: bold;font-size: 17px">极支付收款</p>

												<div class="form-group row">
													<label class="col-md-3 col-form-label" >商户ID</label>
													<div class="col-md-9">
														<input type="text"  name="C_jipay_id" class="form-control"  value="<?php echo $C_jipay_id?>">
													</div>
												</div>

												<div class="form-group row">
													<label class="col-md-3 col-form-label" >商户密钥</label>
													<div class="col-md-9">
														<input type="text"  name="C_jipay_key" class="form-control"  value="" <?php if($C_jipay_key!=""){echo "placeholder=\"已加密保存，留空则不修改\"";}?>>
													</div>
												</div>
												<div class="form-group row">
													<label class="col-md-3 col-form-label" ></label>
													<div class="col-md-9">
														...
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*如果您不知道如何填写，请点击<a href="<?php echo $url_from?>/h37.html" target="_blank">查看帮助</a> 提供平台：<a href="http://jipay.net/" target="_blank">极支付官网</a></div>
													</div>
												</div>

												<hr>

												<p style="text-align: center;font-weight: bold;font-size: 17px">虎皮椒收款</p>

												<div class="form-group row">
													<label class="col-md-3 col-form-label" >AppID</label>
													<div class="col-md-9">
														<input type="text"  name="C_hupay_id" class="form-control"  value="<?php echo $C_hupay_id?>">
													</div>
												</div>

												<div class="form-group row">
													<label class="col-md-3 col-form-label" >密钥</label>
													<div class="col-md-9">
														<input type="text"  name="C_hupay_key" class="form-control"  value="" <?php if($C_hupay_key!=""){echo "placeholder=\"已加密保存，留空则不修改\"";}?>>
													</div>
												</div>
												<div class="form-group row">
													<label class="col-md-3 col-form-label" ></label>
													<div class="col-md-9">
														...
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*如果您不知道如何填写，请点击<a href="<?php echo $url_from?>/h38.html" target="_blank">查看帮助</a> 提供平台：<a href="https://www.xunhupay.com/" target="_blank">虎皮椒官网</a></div>
													</div>
												</div>
												...
												<div class="form-group row">
													<label class="col-md-3 col-form-label" >微信-商户号</label>
													<div class="col-md-9">
												...
												<div class="form-group row">
													<label class="col-md-3 col-form-label" >微信-通信密钥</label>
													<div class="col-md-9">
												...
												<div class="form-group row">
													<label class="col-md-3 col-form-label" >支付宝-商户号</label>
													<div class="col-md-9">
														<input type="text"  name="C_payjs_id2" class="form-control" value="<?php echo $C_payjs_id2?>">
													</div>
												</div>

												<div class="form-group row">
													<label class="col-md-3 col-form-label" >支付宝-通信密钥</label>
													<div class="col-md-9">
														<input type="text"  name="C_payjs_key2" class="form-control" value="" <?php if($C_payjs_key2!=""){echo "placeholder=\"已加密保存，留空则不修改\"";}?>>
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*如果您不知道如何填写，请点击<a href="<?php echo $url_from?>/h20.html" target="_blank">查看帮助</a> 提供平台：<a href="https://payjs.cn/" target="_balnk">payjs官网</a></div>
													</div>
												</div>
														</div>
												...
												<div class="tab-pane fade" id="t6" role="tabpanel" aria-labelledby="contact-tab2">
													<p style="text-align: center;font-weight: bold;font-size: 17px">邮箱接口</p>
												<div class="form-group row">
												...
												<div class="form-group row">
													...
													<div class="col-md-9 buy">
														...
														<label aa="C_mailtype" <?php if($C_mailtype==0){echo "class='checked'";}?> onclick="e_s(0)"><input value="0" type="radio" name="C_mailtype" <?php if($C_mailtype==0){echo "checked='checked'";}?>> 公共邮箱</label>
												...
												<div id="email_set2">
													<div class="form-group row">
													<label class="col-md-3 col-form-label">风险提示</label>
													<div class="col-md-9">
														<p>公共邮箱所有用户共用，为防止负荷过重瘫痪，仅提供发送验证码服务</p>
													</div>
												</div>
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
														...
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*如果您不知道如何填写，请点击<a href="<?php echo $url_from?>/h8.html" target="_blank">查看帮助</a></div>
													</div>
												</div>
												...
												<div class="form-group row">
													<label class="col-md-3 col-form-label"></label>
													<div class="col-md-9">
														<button onclick="testmail()" type="button" class="btn btn-sm btn-info">发送测试邮件</button>
													</div>
												</div>
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
														<div style="margin-top: 5px;"><?php echo $sms_rest?> <button type="button" class="btn btn-info btn-sm" onclick="$('#sms').modal('show');$('#sms_pay').html('<iframe src=\'<?php echo $url_from?>/sms/\' name=\'mapif\' type=\'1\' frameborder=\'0\' height=\'820\' width=\'100%\'></iframe>');">购买短信包</button></div>
													</div>
												</div>
												<div class="form-group row">
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
														...
														（<span class="intro">说明：用户购买商品后提醒管理员/商户发货</span>）
													</div>
												</div>
												<hr>
												<p style="text-align: center;font-weight: bold;font-size: 17px">OSS云存储接口</p>
													...
													<div class="col-md-9">
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">
															*使用OSS云储存功能可以极大的提高图片加载速度并且节省本地空间<br>
															如果您不知道如何填写，请点击<a href="<?php echo $url_from?>/h26.html" target="_blank">查看帮助</a> 提供平台：<a href="https://www.aliyun.com/product/oss" target="_balnk">阿里云OSS云存储</a></div>

													</div>
												</div>
											</div>
											
											<?php if($config->wuliu=="true"){?>
											<hr>
											<p style="text-align: center;font-weight: bold;font-size: 17px">快递接口</p>
											<div class="form-group row">
													<label class="col-md-3 col-form-label" >快递接口AppCode</label>
													<div class="col-md-9">
														<input type="text"  name="C_kd" class="form-control" value="<?php echo $C_kd?>">
													</div>
													</div>
												...
												<div class="form-group row">
													<label class="col-md-3 col-form-label">主站展示 </label>
													<div class="col-md-9 buy">
														<label aa="C_zzk" <?php if($C_zzk==0){echo "class='checked'";}?>><input value="0" type="radio" name="C_zzk" <?php if($C_zzk==0){echo "checked='checked'";}?>> 展示全部内容</label>
														<label aa="C_zzk" <?php if($C_zzk==1){echo "class='checked'";}?>><input value="1" type="radio" name="C_zzk" <?php if($C_zzk==1){echo "checked='checked'";}?>> 仅展示主站内容</label>
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
												        ...
												        <div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*该功能仅适用于站群版授权，请点击<a href="<?php echo $url_from?>/h27.html" target="_blank">查看帮助</a></div>
													</div>
												</div>
												...
												<div class="form-group row">
													...
													<div class="col-md-9 buy">
														...
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*如果开启伪静态，需额外配置文件，请点击<a href="<?php echo $url_from?>/h10.html" target="_blank">查看帮助</a></div>
													</div>
												</div>
													...
													<div class="col-md-9">
												        ...
												        <div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">
															*佣金比例设置为0则不开启该级分销功能<br>
															如果不懂如何设置分销规则，请点击<a href="<?php echo $url_from?>/h16.html" target="_blank">查看帮助</a></div>
													</div>
												</div>

												<div class="form-group row">
													<label class="col-md-3 col-form-label">积分设置</label>
													<div class="col-md-9">
														<p><div class="input-group">
												            <span class="input-group-addon">消费一元获得</span>
												            <input type="text" class="form-control" name="C_fen1" value="<?php echo $C_fen1?>">
												            <span class="input-group-addon">积分</span>
												        </div></p>
												        <p><div class="input-group">
												            <span class="input-group-addon">每日签到获得</span>
												            <input type="text" class="form-control" name="C_fen2" value="<?php echo $C_fen2?>">
												            <span class="input-group-addon">积分</span>
												        </div></p>
												        <p><div class="input-group">
												            <span class="input-group-addon">一积分兑换</span>
												            <input type="text" class="form-control" name="C_fen3" value="<?php echo $C_fen3?>">
												            <span class="input-group-addon">元</span>
												        </div></p>
												        
													</div>
												</div>
												<?php if($config->postage=="true"){?>
												<div class="form-group row">
													<label class="col-md-3 col-form-label">邮费设置</label>
													<div class="col-md-9">
														<p><div class="input-group">
												            <span class="input-group-addon">常规邮费</span>
												            <input type="text" class="form-control" name="C_postage" value="<?php echo $C_postage?>">
												            <span class="input-group-addon">元</span>
												        </div></p>
												        <p><div class="input-group">
												            <span class="input-group-addon">满额包邮</span>
												            <input type="text" class="form-control" name="C_baoyou" value="<?php echo $C_baoyou?>">
												            <span class="input-group-addon">元</span>
												        </div></p>
												        <div style="font-size: 12px;margin-top: 10px;color: #AAAAAA">*说明：邮费仅针对实物商品有效</div>
													</div>
												</div>
												<?php }?>
												...
												<hr>
												<div class="form-group row">
													<label class="col-md-3 col-form-label" >可选模块开关</label>
													<div class="col-md-9">
														<label><input value="true" type="checkbox" name="C_product" <?php if($config->product!="false"){echo "checked='checked'";}?>> 商品模块</label><br>
														<label><input value="true" type="checkbox" name="C_news" <?php if($config->news!="false"){echo "checked='checked'";}?>> 文章模块</label><br>
														<label><input value="true" type="checkbox" name="C_course" <?php if($config->course!="false"){echo "checked='checked'";}?>> 课程模块</label><br>
													</div>
												</div>
												...
												<div class="tab-pane fade" id="t8" role="tabpanel" aria-labelledby="contact-tab2">
													...
													<div class="col-md-9">
														...
														<label><input type="radio" name="C_appt" value="course" <?php if($C_appt=="course"){echo "checked='checked'";}?>> 课程型</label> <button type="button" class="btn btn-info btn-sm" onclick="show('t13')">演示</button>
													</div>
													</div>
													...
													<div class="col-md-9">
														<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*如何不知道如何生成，请点击<a href="<?php echo $url_from?>/h12.html" target="_blank">生成教程</a>和<a href="<?php echo $url_from?>/h13.html" target="_blank">配置教程</a></div>
													</div>
													</div>
												...
												<div class="tab-pane fade" id="t9" role="tabpanel" aria-labelledby="contact-tab2">
													...
													<div class="form-group row">
														<label class="col-md-3 col-form-label">算术验证</label>
														<div class="col-md-9 buy">
															...
															<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*会员中心及后台登录时需进行算术验证，防暴力破解</div>
														</div>
													</div>
													...
													<div class="form-group row">
													<label class="col-md-3 col-form-label" >防恶意购买</label>
													<div class="col-md-9">
														<div class="input-group">
												            <span class="input-group-addon">每次购买间隔</span>
												            <input type="text"  name="C_buylimit" class="form-control" value="<?php echo $C_buylimit?>">
												            <span class="input-group-addon">分钟</span>
												        </div>
												        <div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">说明：填写正整数，填0则不限制 <input type="checkbox" name="C_viplimit" value="1" <?php if($C_viplimit==1){echo "checked='checked'";}?>> 仅限制VIP</div>
													</div>
													</div>
		                        ...
		                        <button class="btn btn-info" type="button" onclick="changeadmin()">修改</button>
												...
												<div class="tab-pane fade" id="t11" role="tabpanel" aria-labelledby="contact-tab2">
													...
													<div class="form-group row">
														...
														<label class="col-md-3 col-form-label">网站地图sitemap</label>
														<div class="col-md-9">
				...
				<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*如果您不知道如何使用以上功能，请点击<a href="<?php echo $url_from?>/h32.html" target="_blank">使用帮助</a></div>
														</div>
														
													</div>
													<hr>
													<div class="form-group row">
														<label class="col-md-3 col-form-label">开启缓存功能</label>
														<div class="col-md-9 buy">
															<label aa="C_cache" <?php if($C_cache==1){echo "class='checked'";}?>><input value="1" type="radio" name="C_cache" <?php if($C_cache==1){echo "checked='checked'";}?>> 开启</label>
															<label aa="C_cache" <?php if($C_cache==0){echo "class='checked'";}?>><input value="0" type="radio" name="C_cache" <?php if($C_cache==0){echo "checked='checked'";}?>> 关闭</label>
															<div style="margin-top: 10px;font-size: 12px;color: #AAAAAA">*开启后网站直接读取缓存文件，可提高加载速度，但是<span style="color: #f00">后台更改设置需清理缓存后才可生效</span></div>
														</div>
													</div>
													<div class="form-group row">
														<label class="col-md-3 col-form-label">清理缓存</label>
														<div class="col-md-9">
															<button class="btn btn-info" type="button" onclick="clear_cache()">清理缓存</button>
															<span id="sitemap_info"></span>
														</div>
													</div>
												</div>
    ...
    function clear_cache(){
		...
		function show(T_id){
			$('#appt').modal('show');
			$('#app_show').html('<iframe src="<?php echo $demo?>/'+T_id+'/wap/" style="border:none;height:700px;width:100%"></iframe>')
		}
		function testmail(){
			$.ajax({
	            	url:'?action=save',
			...
			function creat(){
				...
				$.ajax({
	            	...
	            	if(data.msg=="success"){
	            		...
	            		window.location.href="<?php echo $url_from?>/app/download/"+data.url+".zip";
	            	}else{
	            		toastr.error(data.msg, '错误');
	            	}
	            	}
