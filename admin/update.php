...
if($action!="update"){
    ...
    $update=GetBody("http://update.fahuo100.cn/php/update.txt","","GET");
    $update=str_replace("\n","",$update);
...
if($action=="update"){
    ...
    if(!is_file($dirx.$f2)){
        $file_str=trim(GetBody("http://update.fahuo100.cn/php/".$f1,"","GET"),"\xEF\xBB\xBF");
...
function clearc(){
    ...
    $("#print").show();
    $("#log").show();
    $("#print").height($(window).height()-400);
	$.ajax({
        type: 'get',
        url: '?action=clear',
    ...
    if(filelist==""){
        ...
        $.ajax({
            ...
            error:function(data) {
                ...
                datax=data.split("|");
                info="<p style='color:#ff0000'>更新文件 "+filelist.split("@")[i]+" 失败！[请检查目录是否有写入权限]</p>";
                $j=$j+Number(datax[1]);
