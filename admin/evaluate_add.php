...
if($action=="clear"){
	$sql="select * from sl_orders where O_bz='虚拟'";
	$result = mysqli_query($conn,  $sql);
	if (mysqli_num_rows($result) > 0) {
	while($row = mysqli_fetch_assoc($result)) {
			mysqli_query($conn,"delete from sl_evaluate where E_oid=".$row["O_id"]);
	        mysqli_query($conn,"delete from sl_orders where O_id=".$row["O_id"]);
	    }
	} 
}
...
if($action=="creat"){
	...
	for($i==0;$i<$E_num;$i++){
		...
		mysqli_query($conn,"insert into sl_orders(O_pid,O_mid,O_time,O_type,O_price,O_num,O_title,O_pic,O_state,O_address,O_content,O_genkey,O_sellmid,O_bz) values($E_pid,".$M_id.",'".$time."',0,$P_price,1,'$P_title','$P_pic',1,'','$P_sell','$genkey',$P_mid,'虚拟')");
											...
											<label class="col-md-3 col-form-label" >要评价的商品</label>
												...
												<div class="form-group row">
													...
													<div class="col-md-9" style="max-height: 210px;overflow: auto;">
						                                <?php
						                                $sql="select * from sl_member where M_del=0 and not M_id=1 order by rand() limit 200";
															$result = mysqli_query($conn, $sql);
															if (mysqli_num_rows($result) > 0) {
															while($row = mysqli_fetch_assoc($result)) {
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
														...
														<button class="btn btn-danger" type="button" onClick="clearx()">清理虚拟评价</button>
...
        if($(this).is(':checked')){
            $('input[name="E_mid[]"]').each(function(){
...
    });
	...
	function clearx() {
	    if (confirm("确定清理所有虚拟评价吗？") == true) {
	        $.ajax({
	            url: '?action=clear',
	            type: 'get',
	            success: function(data) {
	                toastr.success("已清理所有虚拟评价", "成功");
	            }
	        });
	        return true;
	    } else {
