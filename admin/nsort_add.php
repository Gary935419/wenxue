...
if($S_title!=""){
	mysqli_query($conn,"insert into sl_nsort(S_pic,S_icon,S_title,S_content,S_order,S_sub,S_show,S_keywords) values('$S_pic','$S_icon','$S_title','$S_content',$S_order,$S_sub,$S_show,'$S_keywords')");

	$sub=getrs("select * from sl_nsort where S_sub=0 and S_title='$S_title' order by S_id desc","S_id");
	if($S_sub==0){
		mysqli_query($conn,"insert into sl_nsort(S_pic,S_icon,S_title,S_content,S_order,S_sub,S_show,S_keywords) values('$S_pic','$S_icon','默认子分类','默认子分类',0,$sub,$S_show,'默认子分类')");
	}
				...
				<div class="app-content">
					<section class="section">
                    	<ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active"><a href="nsort_add.php">文章分类</a></li>
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
														<select name="S_sub" class="form-control">
															...
															<?php
															$sql="select * from sl_nsort where S_del=0 and S_sub=0 and not S_id=$S_id order by S_order,S_id desc";
																$result = mysqli_query($conn, $sql);
																if (mysqli_num_rows($result) > 0) {
																while($row = mysqli_fetch_assoc($result)) {
																	if($S_sub==$row["S_id"]){
