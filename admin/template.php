...
$template=json_decode(file_get_contents($updateurl."/template/template.json"),true);
...
flush();
ob_flush();
$url=$updateurl."/template/fahuo_".$T_id.".xml";
...
while( ($filename = readdir($handler)) !== false ){
 if(is_dir("../template/".$filename) && $filename != "." && $filename != ".."){  
 	...
 	if(!is_file("../template/".$filename."/version.txt")){
 		$T_version="未标识";
 	}else{
 		$T_version=file_get_contents("../template/".$filename."/version.txt");
 	}

 	echo "<tr><td>".$filename."</td><td>模板名称：".getValueByKey(json_encode($template),array('T_id',$filename),'T_title')."<br>展示类型：".getValueByKey(json_encode($template),array('T_id',$filename),'T_type')."<br>模板版本：".$T_version."</td><td><a href=\"tpl_edit.php?id=".$filename."\"><img src=\"".$url_from."/images/".$filename.".jpg\" height=\"80\" style=\"margin-right:10px;margin-bottom:5px;box-shadow: 0 2px 17px 2px rgb(222, 223, 241);\" alt=\"<img src='".$url_from."/images/".$filename.".jpg' width='500'>\"></a></td><td class=\"buy\"><label aa=\"C_template\" class=\"".$class."\"><input type=\"radio\" value=\"".$filename."\" name=\"C_template\" ".$checked." onclick=\"change()\">●</label></td><td class=\"buy\"><label aa=\"C_wap\" class=\"".$class2."\"><input type=\"radio\" value=\"".$filename."\" name=\"C_wap\" ".$checked2." onclick=\"change()\">●</label></td>";
...
foreach($template as $v) {
	if(strpos($v["T_type"],"PC")!==false){
		...
		<div class=\"tbox \">
		<a href=\"".$url_from."/template_show.html?T_id=".$v["T_id"]."\" target=\"_blank\"><img src=\"".$url_from."/images/".$v["T_id"].".jpg\" alt=\"<img src='".$url_from."/images/".$v["T_id"].".jpg' width='500'>\"></a>
		<b>".$v["T_id"]."</b>-".$v["T_title"]."
		".$T_info."
		<div>线上版本：".$v["version"]."</div>
		</div>
...
foreach($template as $v) {
	if(strpos($v["T_type"],"WAP")!==false){
		...
		echo "<div class=\"col-md-4\">
		<div class=\"tbox \">
		<a href=\"".$url_from."/template_show.html?T_id=".$v["T_id"]."\" target=\"_blank\"><img src=\"".$url_from."/images/".$v["T_id"]."_wap.jpg\"></a>
		<b>".$v["T_id"]."</b>-".$v["T_title"]."
		".$T_info."
		<div>线上版本：".$v["version"]."</div>
		</div>
