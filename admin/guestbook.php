...
if($action=="reply"){
	if($_POST["G_reply"]!=""){
		...
		$info=sendmail("您的留言有回复","<p>留言标题：".$G_title."</p><p>留言时间：".$G_time."</p><p>留言内容：".$G_msg."</p><p>留言回复：".$_POST["G_reply"]."</p>",$G_mail);
		if($info=="success"){
													...
													<?php 
														$sql="select * from sl_guestbook where G_del=0 order by G_id desc limit 20";
																...
																while($row = mysqli_fetch_assoc($result)) {
																	...
																	<div class=\"part\"><input type=\"checkbox\" name=\"id[]\" value=\"".$row["G_id"]."\"> ".htmlspecialchars($row["G_title"])."</div> 
