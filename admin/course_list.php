...
if($action=="del"){
	mysqli_query($conn,"update sl_course set C_del=1 where C_id=".$C_id);
	mysqli_query($conn, "insert into sl_log(L_aid,L_time,L_add,L_ip,L_title) values(".$_SESSION["A_id"].",'".date('Y-m-d H:i:s')."','".$_SESSION["add"]."','".getip()."','删除课程')");
	die("success");
}

if($action=="dels"){
	mysqli_query($conn,"update sl_usort set S_del=1 where S_id=".$S_id." or S_sub=".$S_id);
	mysqli_query($conn,"update sl_course set C_del=1 where C_sort=".$S_id." or C_sort in (select S_id from sl_usort where S_sub=".$S_id.")");
	mysqli_query($conn, "insert into sl_log(L_aid,L_time,L_add,L_ip,L_title) values(".$_SESSION["A_id"].",'".date('Y-m-d H:i:s')."','".$_SESSION["add"]."','".getip()."','删除课程分类')");
	die("success");
}
...
if($action=="save"){
	foreach ($_POST as $x=>$value) {
		if(splitx($x,"_",0)=="title"){
			mysqli_query($conn,"update sl_course set C_title='".$_POST[$x]."' where C_id=".intval(splitx($x,"_",1)));
		}
		if(splitx($x,"_",0)=="order"){
			mysqli_query($conn,"update sl_course set C_order=".$_POST[$x]." where C_id=".intval(splitx($x,"_",1)));
		}
		if(splitx($x,"_",0)=="sort"){
			mysqli_query($conn,"update sl_course set C_sort=".$_POST[$x]." where C_id=".intval(splitx($x,"_",1)));
		}
		if(splitx($x,"_",0)=="price"){
			mysqli_query($conn,"update sl_course set C_price=".$_POST[$x]." where C_id=".intval(splitx($x,"_",1)));
		}
	}
	die("success");
}
...
if($action=="delall"){
	...
	if(count($id)>0) {
		...
		for ($i=0 ;$i<count($id);$i++ ) {
			mysqli_query($conn,"update sl_course set C_del=1 where C_id=".intval($id[$i]));
			$shu=$shu+1 ;
			$ids=$ids.$id[$i].",";
		}
		$ids= substr($ids,0,strlen($ids)-1);
		if($shu>0) {
			mysqli_query($conn, "insert into sl_log(L_aid,L_time,L_add,L_ip,L_title) values(".$_SESSION["A_id"].",'".date('Y-m-d H:i:s')."','".$_SESSION["add"]."','".getip()."','批量删除课程')");
			die("success");
		} else {
			die("删除失败");
		}
...
$C_all=getrs("select count(C_id) as C_count from sl_course where C_del=0","C_count");
?>
		...
		<div id="app">
			<div class="main-wrapper" >
				...
				<div class="app-content">
					...
					<a href="course_add.php" class="btn btn-primary pull-right btn-sm" style="z-index: 2;position: relative;margin-right: 5px;"><i class="fa fa-plus-circle"></i> 新增课程</a>
							...
							<div class="row">
								...
								<div class="col-lg-4">
									...
									<div class="card card-primary">
										...
										<ul class="list-group">
											<a href="?S_id=0" class="list-group-item <?php if($S_id==0){echo "active";}?>"><b>所有课程 [<?php echo $C_all?>个]</b>
											...
											<?php 
													$sql="select * from sl_usort where S_del=0 and S_sub=0 order by S_order,S_id desc";
														...
														while($row = mysqli_fetch_assoc($result)) {
															...
															$C_count=getrs("select count(C_id) as C_count from sl_course,sl_usort where C_sort=S_id and S_sub=".$row["S_id"]." and C_del=0","C_count");
															echo "<div style=\"position:relative;border-top:solid 1px #EEEEEE;\" id=\"s".$row["S_id"]."\"><a href=\"?S_id=".$row["S_id"]."\" class=\"list-group-item ".$active."\"><b>└ ".$row["S_order"].".".$row["S_title"]." [".$C_count."个] </b>".$show."</a><a href=\"usort_add.php?S_id=".$row["S_id"]."\" style=\"position:absolute;right:55px;top:10px;z-index:999;font-size:15px\" class=\"btn btn-sm btn-info\"><i class=\"fa fa-edit\"></i></a> <button style=\"position:absolute;right:10px;top:10px;z-index:999;font-size:15px\" class=\"btn btn-sm btn-danger\" onClick=\"dels(".$row["S_id"].")\"><i class=\"fa fa-times-circle\"></i></button></div>";
															...
															$sql2="select * from sl_usort where S_del=0 and S_sub=".$row["S_id"]." order by S_order,S_id desc";
																...
																while($row2 = mysqli_fetch_assoc($result2)) {
																	...
																	$C_count2=getrs("select count(C_id) as C_count from sl_course where C_del=0 and C_sort=".$row2["S_id"],"C_count");
																	echo "<div style=\"position:relative;border-top:solid 1px #EEEEEE;\" id=\"s".$row2["S_id"]."\"><a href=\"?S_id=".$row2["S_id"]."\" class=\"list-group-item ".$active2."\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└ ".$row2["S_order"].".".$row2["S_title"]." [".$C_count2."个] ".$show."</a><a href=\"usort_add.php?S_id=".$row2["S_id"]."\" style=\"position:absolute;right:55px;top:10px;z-index:999;font-size:15px\" class=\"btn btn-sm btn-info\"><i class=\"fa fa-edit\"></i></a> <button style=\"position:absolute;right:10px;top:10px;z-index:999;font-size:15px\" class=\"btn btn-sm btn-danger\" onClick=\"dels(".$row2["S_id"].")\"><i class=\"fa fa-times-circle\"></i></button></div>";
...
<?php
		...
		if($row["C_top"]==1){
			...
			<td><input type=\"txt\" style=\"width:50px;\" class=\"form-control\" name=\"order_".$row["C_id"]."\" value=\"".$row["C_order"]."\" onblur=\"save()\">".$top."</td>
			<td><textarea style=\"width:100%;min-width:180px;\" rows=\"3\" class=\"form-control\" name=\"title_".$row["C_id"]."\" onblur=\"save()\"/>".htmlspecialchars($row["C_title"])."</textarea></td>
			<td>
			<input type=\"text\" class=\"form-control\" style=\"min-width:80px;max-width:120px\" name=\"price_".$row["C_id"]."\" value=\"".$row["C_price"]."\" onblur=\"save()\">
			</td>
			<td><img src=\"".pic2($row["C_pic"])."\" height=\"50\" alt=\"<img src='".pic2($row["C_pic"])."' width='500'>\"></td>
			<td><p><select name=\"sort_".$row["C_id"]."\" class=\"form-control\" style=\"width:120px;\" onchange=\"save()\">";
													$sql2="select * from sl_usort where S_del=0 and S_sub=0 order by S_order,S_id desc";
																	$result2 = mysqli_query($conn, $sql2);
																	if (mysqli_num_rows($result2) > 0) {
																	while($row2 = mysqli_fetch_assoc($result2)) {
																		echo "<optgroup label=\"".$row2["S_title"]."\">";
																		$sql3="select * from sl_usort where S_del=0 and S_sub=".$row2["S_id"]." order by S_order,S_id desc";
													...
													echo "</select><p><p>
<a class=\"btn btn-sm btn-warning\" target=\"_blank\" href=\"../?type=courseinfo&id=".$row["C_id"]."\"><i class=\"fa fa-send\"></i> 浏览</a>
<button class=\"btn btn-primary btn-sm\" type=\"button\" onclick=\"\$('#haibao').modal('show');\$('#poster').html('<iframe src=\'poster.php?type=course&id=".$row["C_id"]."\' scrolling=\'no\' name=\'mapif\' type=\'1\' frameborder=\'0\' height=\'820\' width=\'100%\'></iframe>')\"><i class=\"fa fa-image\"></i> 海报</button></p>
													</td>
			
			<td><p><a href='course_add.php?C_id=".$row["C_id"]."' class='btn btn-sm btn-info'><i class=\"fa fa-edit\"></i> 编辑</a></p><p><button class='btn btn-sm btn-danger' type='button' onClick='del(".$row["C_id"].")'><i class=\"fa fa-times-circle\"></i> 删除</button></p></td>
...
function loadpage(id) {
    ...
    $.jqPaginator('#pagination', {
        ...
        onPageChange: function (num, type) {
            if (type == "change") {
                window.location="course_list.php?S_id="+<?php echo $S_id?>+"&page="+num;
            }
        }
    });
}
...
function dels(id){
			if (confirm("确定删除课程分类吗（分类下课程也会删除）？")==true){
                $.ajax({
