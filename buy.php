...
if($type=="courseinfo"){
	...
	if($_SESSION["M_id"]!=""){
		...
		if(getrs("select O_id from sl_orders where O_genkey='$genkey'","O_id")==""){//判断订单是否已存在
			mysqli_query($conn, "insert into sl_orders(O_cid,O_mid,O_time,O_type,O_price,O_num,O_title,O_pic,O_state,O_address,O_content,O_genkey,O_sellmid) values($id,$M_id,'".date('Y-m-d H:i:s')."',2,$money,1,'$C_title','$C_pic',0,'','$page','$genkey',$C_mid)");
		}
		if($C_buylimit>0){
...
if($type=="newsinfo"){
	...
	if(getrs("select O_id from sl_orders where O_genkey='$genkey'","O_id")==""){//判断订单是否已存在
		mysqli_query($conn, "insert into sl_orders(O_nid,O_mid,O_time,O_type,O_price,O_num,O_title,O_pic,O_state,O_address,O_content,O_genkey,O_sellmid) values($id,$M_id,'".date('Y-m-d H:i:s')."',1,$money,1,'$N_title','$N_pic',0,'','$genkey','$genkey',$N_mid)");
	}
	if($C_buylimit>0){
		$_SESSION["buylimit"]=time();
	}
...
if($type=="productinfo"){
	...
	if(splitx($P_taobao,"|",0)!=""){
		...
		if($P_unlogin==1 || $_SESSION["M_id"]!=""){
        	...
        	if($gg_rest>0){
        		if(getrs("select O_id from sl_orders where O_genkey='$genkey'","O_id")==""){//判断订单是否已存在
        			mysqli_query($conn, "insert into sl_orders(O_pid,O_mid,O_time,O_type,O_price,O_num,O_content,O_title,O_pic,O_address,O_state,O_genkey,O_sellmid,O_gg) values($id,$M_id,'".date('Y-m-d H:i:s')."',0,$money,$num,'','$P_title','$P_pic','',0,'$genkey',$P_mid,'$sc')");
        		}else{
