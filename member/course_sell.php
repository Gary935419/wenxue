...
if($action=="del"){
	mysqli_query($conn,"update sl_course set C_del=1 where C_id=$C_id and C_mid=$M_id");
	die("success");
}

$sql="select count(C_id) as C_count from sl_course,sl_usort where C_del=0 and S_del=0 and C_sort=S_id and C_mid=".$_SESSION["M_id"];
...
<?php require 'top.php';?>
		...
		<ul class="nav nav-pills nav-stacked">
	        <li class="active"><a href="course_sell.php">课程列表</a></li>
	        <li><a href="course_add.php">新增课程</a></li>
	     </ul>
					</div>
					<div class="col-sm-10 b-left">
						...
						<div class="panel panel-default">
							...
							我发布的课程
							<a href="course_add.php"  class="btn btn-info btn-xs">新增课程</a>
							<a href="../?type=course&M_id=<?php echo $_SESSION["M_id"]?>" target="_blank" class="pull-right btn btn-primary btn-xs">浏览全部课程</a>
						</div>
							...
							$sql="select * from sl_course,sl_usort where C_del=0 and S_del=0 and C_sort=S_id and C_mid=".$_SESSION["M_id"]." order by C_id desc limit ".(($page-1)*10).",10";
							$result = mysqli_query($conn,  $sql);
							...
							while($row = mysqli_fetch_assoc($result)) {
							        ...
							        echo "<tr id=\"".$row["C_id"]."\">
							        <td><a href=\"../?type=courseinfo&id=".$row["C_id"]."\" target=\"_blank\">".$row["C_title"]."</a> <a href=\"../?type=courseinfo&id=".$row["C_id"]."\" target=\"_blank\" class=\"btn btn-xs btn-info\"><i class=\"fa fa-link\"></i> 链接</a></td>
							        <td>".$row["C_price"]."元</td>
							        <td>".$row["S_title"]."</td>
							        <td><img src=\"".pic2($row["C_pic"])."\" height=\"50\"></td>
							        <td>".$sh."</td>
							        <td><a href=\"course_add.php?C_id=".$row["C_id"]."\" class=\"btn btn-xs btn-success\"><i class=\"fa fa-edit\"></i> 编辑</a></td>
							        <td><a href=\"javascript:;\" onClick=\"del(".$row["C_id"].")\" class=\"btn btn-xs btn-danger\"><i class=\"fa fa-times\"></i> 删除</a></td>
        ...
        <input type="hidden" id="visiblePages" runat="server" value="7" />
				...
				<div style="float: right;margin-top: -10px;">说明：课程通过审核后方可在前台显示</div>
			</div>
			</div>
			</div>
