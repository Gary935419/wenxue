...
if($action=="found"){
    ...
    if(($_POST["M_code"]!=$_SESSION["CmsCode"] || $_POST["M_code"]=="" || $_SESSION["CmsCode"]=="") && $C_slide==1){
        ...
        $sql="select * from sl_member where M_email='".t($M_email)."'";
            ...
            $info=sendmail("找回密码邮件","请点击链接重新设置密码<br><a href='http://".$D_domain."/member/setpwd.php?M_pwdcode=".$M_pwdcode."'>http://".$D_domain."/member/setpwd.php?M_pwdcode=".$M_pwdcode."</a><br>说明：重置密码后链接失效",$M_email);
            if($info=="success"){
                box("请查收密码找回邮件!","login.php","success");
            }else{
                box("请".$info."秒后再试！","login.php","success");
            }
        }else{
