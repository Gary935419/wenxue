...
if($action=="edit"){
	...
	$M_postage=round($_POST["M_postage"],2);
	$M_baoyou=round($_POST["M_baoyou"],2);
	//$M_code=$_POST["M_code"];
	...
	$sql="select * from sl_product where P_mid=0";
	...
	$sql="select * from sl_news where N_mid=0";
	$result = mysqli_query($conn,  $sql);
	if (mysqli_num_rows($result) > 0) {
		while($row = mysqli_fetch_assoc($result)) {
	...
	if($M_webtitle!=""){
		mysqli_query($conn,"update sl_member set M_webtitle='".$M_webtitle."',M_keyword='".$M_keyword."',M_description='".$M_description."',M_logo='".$M_logo."',M_ico='".$M_ico."',M_priceup=".$M_priceup.",M_show=".$M_show.",M_domain='".$M_domain."',M_beian='".$M_beian."',M_copyright='".$M_copyright."',M_notice='".$M_notice."',M_postage=".$M_postage.",M_baoyou=".$M_baoyou." where M_id=".$M_id);
...
<form id="userinfo_save" method="POST" action="?action=edit" class="form-horizontal" id="form">
							...
							<div class="form-group">
								<label for="oldpass" class="col-sm-2 control-label">邮费设置</label>
								<div class="col-sm-6">
									<p><div class="input-group">
							            <span class="input-group-addon">常规邮费</span>
							            <input type="text" class="form-control" name="M_postage" value="<?php echo $M_postage?>">
							            <span class="input-group-addon">元</span>
							        </div></p>
							        <p><div class="input-group">
							            <span class="input-group-addon">满额包邮</span>
							            <input type="text" class="form-control" name="M_baoyou" value="<?php echo $M_baoyou?>">
							            <span class="input-group-addon">元</span>
							        </div></p>
							        <div style="font-size: 12px;margin-top: 10px;">*说明：邮费仅针对实物商品有效</div>
								</div>
							</div>
							<?php }?>
