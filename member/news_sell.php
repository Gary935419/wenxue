...
$N_all=getrs("select count(N_id) as N_count from sl_news where N_del=0 and N_mid=$M_id","N_count");
?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
		...
		<div class="container m_top_30">
			<div class="yto-box">
				<div class="row">
					...
					<div class="col-sm-3 hidden-xs b-right">
			<h5 class="p_bottom_10">文章分类</h5>
		<ul class="nav nav-pills nav-stacked">
<li class=" <?php if($S_id==0){echo "active";}?>"><a href="?S_id=0" ><b>所有文章 [<?php echo $N_all?>个]</b></a>
			<?php
													...
													$sql="select * from sl_nsort where S_del=0 and S_sub=0 order by S_order,S_id desc";
														...
														while($row = mysqli_fetch_assoc($result)) {
															...
															$N_count=getrs("select count(N_id) as N_count from sl_news,sl_nsort where N_sort=S_id and S_sub=".$row["S_id"]." and N_del=0 and N_mid=$M_id","N_count");


															echo "<li class=\"".$active."\" id=\"s".$row["S_id"]."\"><a href=\"?S_id=".$row["S_id"]."\"><b>└ ".$row["S_order"].".".$row["S_title"]."  [".$N_count."个]</b>"."</a> </li>";
															...
															$sql2="select * from sl_nsort where S_del=0 and S_sub=".$row["S_id"]." order by S_order,S_id desc";
																...
																while($row2 = mysqli_fetch_assoc($result2)) {
																	...
																	$N_count2=getrs("select count(N_id) as N_count from sl_news where N_del=0 and N_sort=".$row2["S_id"]." and N_mid=$M_id","N_count");

					
																	echo "<li class=\"".$active2."\" id=\"s".$row2["S_id"]."\"><a href=\"?S_id=".$row2["S_id"]."\" >&nbsp;&nbsp;&nbsp;&nbsp;└ ".$row2["S_order"].".".$row2["S_title"]." [".$N_count2."个]"."</a></li>";
...
<a href="../?type=news&M_id=<?php echo $_SESSION["M_id"]?>" target="_blank" class="pull-right btn btn-primary btn-xs">浏览全部文章</a>
				...
				if($S_id==0){
					$sql="select * from sl_news,sl_nsort where N_del=0 and S_del=0 and N_sort=S_id and N_mid=$M_id $order limit ".(($page-1)*10).",10";
				}else{
					$sql="select * from sl_news,sl_nsort where N_del=0 and S_del=0 and N_sort=S_id and N_mid=$M_id and (S_id=".$S_id." or S_sub=".$S_id.") $order limit ".(($page-1)*10).",10";
				}
							...
							while($row = mysqli_fetch_assoc($result)) {
							        ...
							        echo "<tr id=\"".$row["N_id"]."\">
							        <td><a href=\"../?type=newsinfo&id=".$row["N_id"]."\" target=\"_blank\">".$row["N_title"]."</a> <a href=\"../?type=newsinfo&id=".$row["N_id"]."\" target=\"_blank\" class=\"btn btn-xs btn-info\"><i class=\"fa fa-link\"></i> 链接</a></td>
							        <td>".$row["S_title"]."</td>
							        <td>".round($row["N_price"],2)."元</td>
							        <td><img src=\"".pic2($row["N_pic"])."\" height=\"50\"></td>
							        <td>".$sh."</td>
...
function loadpage(id) {
    ...
    $.jqPaginator('#pagination', {
        ...
        onPageChange: function (num, type) {
            if (type == "change") {
                window.location="?S_id=<?php echo $S_id?>&page="+num;
            }
        }
    });
}
