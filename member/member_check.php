...
if ($_SESSION["M_login"]=="" || $_SESSION["M_pwd"]=="" || $_SESSION["M_id"]=="" || $_SESSION["M_pwdcode"]==""){
	...
	if (mysqli_num_rows($result) > 0) {
		...
		$M_postage=$row["M_postage"];
		$M_baoyou=$row["M_baoyou"];
    	...
    	$_SESSION["M_pwd"] = "";
    	$_SESSION["M_pwdcode"] = "";
		die("<script>alert('未发现有该账号！');window.location.href='login.php'</script>");
	}
}

if((strpos($C_regr,"email")!==false && $M_email=="" || strpos($C_regr,"mobile")!==false && $M_mobile=="" || strpos($C_regr,"qq")!==false && $M_qq=="") && strpos(strtolower($_SERVER['PHP_SELF']),"member/edit.php")===false){
	die("<script>alert('您有信息待完善！');window.location.href='edit.php'</script>");
}
