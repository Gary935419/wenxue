								...
								<table class="table table-condensed" style="font-size: 12px;">
								 <thead>
									<tr>
										<th width="10%">图片</th>
										<th width="25%">名称</th>
										<th width="25%">单号/买家信息</th>
										<th width="10%">发货内容/状态</th>
									</tr>
									</thead>
									<tbody>
									<?php

							$sql="select * from sl_orders,sl_list where O_genkey=L_genkey and O_state>0 and O_del=0 and O_sellmid=".$M_id." order by O_id desc limit ".(($page-1)*10).",10";
							$result = mysqli_query($conn,  $sql);
							if (mysqli_num_rows($result) > 0) {
							while($row = mysqli_fetch_assoc($result)) {
								$O_id=$row["O_id"];
								$O_content="<p>".$row["O_content"]."</p>";
								...
								if($row["O_state"]==1){
									$O_state="已发货";
									$th="<a href=\"unlogin.php?type=fahuo&genkey=".$row["O_genkey"]."\" class=\"btn btn-xs btn-primary\" target=\"_blank\">提货</a><br>";
								}
								if($row["O_state"]==2){
								...
								if($row["O_wl"]!=""){
									$wl="<p>快递公司：".$row["O_wl"]."</p><p>快递单号：".$row["O_wlno"]."</p>";
								}else{
									$wl="";
								}
							        ...
							        <td>
							        <p><a href=\"".$url."\" target=\"_blank\">[".$O_type."] <b>".$row["O_title"]."</b> </a></p>
							        <p>".$row["O_price"]."元 × ".$row["O_num"]."=<span style=\"font-weight:bold;color:#ff0000\">".round($row["O_num"]*$row["O_price"],2)."元</span>【".str_replace("|"," ",$row["O_gg"])."】</p>
							        <p>".$row["O_time"]."</p>
							        </td>
							        <td>
							        	<p>".$row["O_paytype"]."</p>
							        	<p>".$row["L_no"]."</p>
							        	<p>".str_replace("__","<br>",$row["O_address"])."</p>
							        </td>
