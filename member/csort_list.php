					...
					<div class="col-sm-10 b-left">
						...
						<div class="panel panel-default">
							...
							while($row = mysqli_fetch_assoc($result)) {
								$rest=getrs("select count(C_id) as C_count from sl_card where C_del=0 and C_sort=".$row["S_id"]." and C_use=0","C_count");
								$all=getrs("select count(C_id) as C_count from sl_card where C_del=0 and C_sort=".$row["S_id"]."","C_count");
								
							        echo "<tr id='".$row["S_id"]."'>
							        <td>".$row["S_id"]."</td>
							        <td>".$row["S_title"]."</td>
							        <td>".$rest."/".$all." <a href=\"card_add.php?id=".$row["S_id"]."\" class=\"btn btn-info btn-xs\"><i class=\"fa fa-plus-square\"></i> 补货</a></td>

							        <td><a href='csort_add.php?S_id=".$row["S_id"]."' class='btn btn-xs btn-success'><i class=\"fa fa-edit\"></i> 编辑</a></td>
